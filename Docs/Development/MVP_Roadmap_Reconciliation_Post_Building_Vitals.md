...
| Production HUD | **PARTIAL — DATA FOUNDATION + VIEWMODEL BRIDGE + RUNTIME BOOTSTRAP + THREAT NORMALIZED + PLANET FERRONITE + LAUNCH MENU PRESENTATION + SELECTION VM FOUNDATION + CONTEXT ACTION FOUNDATION** | Data foundation and HUD bootstrap on `main`. MatchVM exposes presentation-only `FerroniteThreatNormalized` = Clamp(threat / (MainBase `GetTotalCapacity()` × `GetThreatPerStoredUnit()`), 0, 1). ResourceVM exposes exact `PlanetFerronite` = local MainBase `UGP_StorageComponent::GetTotalStored()`. Native right-side Launch Menu presentation: `UGP_LaunchMenuPresenter` on `UGP_HUDViewModelSubsystem`; `UGP_HUDRootWidget` exposes container rows / ready count / Launch request forwarding to `AGP_PlayerController::RequestLaunchReadyContainer`. Native Selection presentation: `UGP_SelectionViewModel` + `UGP_SelectionViewModelAdapter` owned by `UGP_HUDViewModelSubsystem`; None/Single/Group; inspect fallback; GAS health push; Worker cargo push via `OnCargoAmountChanged`; group rows via HUD-root accessor + `BP_OnSelectionPresentationChanged`. Manual MVVM slot name `GP_SelectionViewModel`. Event-driven, no Tick, no world scan. Gameplay selection cap remains 24. No icon field (none on `UGP_UnitDefinition`). Operator PIE **PASSED** (2026-08-25): authored Single Selection bindings in local `WBP_GP_HUD` (Worker / Salvage Walker / MainBase, CurrentHealth, Damage, live Worker CargoAmount). Group visual grid is not authored/operator-validated. Native Context Action Grid presentation exists on `ui/gp-bottom-hud` (`UGP_ContextActionPresenter`; None/Unit/UnitGroup/Building/MainBase; Stop/Attack-Move/Purchase seams; Move/Patrol disabled). Authored Context Action buttons and Group icon grid are **not** operator-validated. This is an **intermediate Bottom HUD checkpoint, not merge-ready**. `WBP_GP_HUD` remains uncommitted. Remaining: MainBase PURCHASE categories, minimap, Patrol backend, notifications, and production end-of-match. TEMP HUD is retired. Production HUD is the active match HUD. |
...
| S49-S53 | Resource/Match VMs, adapters, local-player ownership, widget/HUD-root bases **DONE — FOUNDATION**; native right-side Launch Menu presentation **DONE** (`UGP_LaunchMenuPresenter`); authored HUD/minimap/MainBase PURCHASE panel and remaining panel VMs **REMAINING**. Global Order Menu is superseded as the production HUD path. TEMP HUD is retired. Production HUD is the active match HUD. |
