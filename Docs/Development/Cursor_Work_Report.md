# Cursor Work Report вЂ” GP-S33M RTS Movement Reconciliation
...
| CargoFull в†’ `HaulReturnToBase` | **PASS** (all workers) |
| Haul approach candidate generation | **FAIL** вЂ” `HaulApproachNoReachableCandidate` before any candidate loop |
...
Old 3D budget: `MaxHorizontal = sqrt(400ВІ в€’ 280ВІ) в‰€ 285.7` < `MinOutside(228.2) + Acc + Safety (303.2)` в†’ geometry false, CandidateCount stays 0.

## Formula bug
`TryComputeDesiredHorizontalDistance` used `DeltaZ = PathStart.Z в€’ TargetLocation.Z` and `MaxHorizontalBudget = sqrt(RangeВІ в€’ DeltaZВІ)`.
...
## Fix вЂ” GroundPlane2D building semantics
...
## Contracts
1. **`gp.Resource.RunHaulNavApproachContractTest`** вЂ” operator numbers (Clearanceв‰€218.2, Range=400, Acc=50, Safety=25) + elevated MainBase Z; old 3D fails; GroundPlane2D CandidateCount=8; Dist2D-in-range / Dist3D-out-of-range unload via storage delta; alternate-candidate + all-unreachable cases retained.
2. **`gp.Resource.RunMineReassignmentHaulContractTest`** вЂ” elevated MainBase Z + clearance ~218.2; natural chain unload still PASS.
...
## Operator retest
Same map / MainBase / NavigationObstacle вЂ” no operator tuning.  
Expected: `HaulApproachCandidateвЂ¦` в†’ `HaulApproachSelectedвЂ¦` в†’ move в†’ arrival в†’ `HaulDropOffComplete`.
