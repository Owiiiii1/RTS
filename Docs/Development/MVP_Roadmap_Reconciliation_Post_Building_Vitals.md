...
| Minimap | **SURFACE — INTERMEDIATE** | Native `UGP_MinimapPresenter` on `UGP_HUDViewModelSubsystem` binds the trusted `UGP_LocalFoWComponent`. `UGP_MinimapWidget` paints a static authored background (`UGP_UIPresentationSettings::MinimapBackgroundTexture`, async) plus a bounded FoW overlay (default 128²). Not SceneCapture / render-target terrain. No blips, camera rectangle, click-to-pan, last-known actors, Tick, or 2000×2000 texture copy. Not merge-ready; operator inserts the widget into protected `WBP_GP_HUD`. |
...
| S49-S53 | Resource/Match VMs, adapters, local-player ownership, widget/HUD-root bases **DONE — FOUNDATION**; native right-side Launch Menu presentation **DONE** (`UGP_LaunchMenuPresenter`); Bottom HUD **DONE** on `main`; native minimap surface **IN PROGRESS** on `ui/gp-minimap` (`UGP_MinimapPresenter` + `UGP_MinimapWidget` background/FoW; not a complete minimap). Authored HUD blips/camera and remaining panel VMs **REMAINING**. Global Order Menu is superseded as the production HUD path. TEMP HUD is retired. Production HUD is the active match HUD. |
