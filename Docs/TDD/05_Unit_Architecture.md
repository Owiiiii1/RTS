...
- **GP-S41M:** yaw-only `bRotateToMovement` facing uses shortest signed delta (`FindDeltaAngleDegrees` / `NormalizeAxis`). Not `RInterpConstantTo` on `FRotator` (that helper does not wrap yaw).
