...
**Поточна фаза:** Configuration data ownership cleanup. **Current: UNIT_PAYLOAD_COMPAT_CLEANUP_FINALIZED_READY_FOR_MERGE**.
...
| **Cleanup Slice D** Unit payload compatibility | **FINALIZED READY FOR MERGE** — [`GP-Unit-Payload-Compat-Cleanup.md`](GP-Unit-Payload-Compat-Cleanup.md) — **NOT MERGED** |

**NEXT action для агента:** wait for merge authorization. Do **not** start building payload / timing cleanup. Do **not** migrate authored assets. Do **not** start GP-S42B / GP-S42C / FoW.
