...
| Visual world/terrain FoW | **PARTIAL — OPERATOR VALIDATION PENDING** | Per-LocalPlayer post-process Known/Visible texture mask (1024², spatial + temporal filter) from the trusted mirror. Gameplay FoW remains 200 cm / 5 Hz. Contract and Editor build pass; PIE readability, camera stability, and two-player isolation are the current gate. |
