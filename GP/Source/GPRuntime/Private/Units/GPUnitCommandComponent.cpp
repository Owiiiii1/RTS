...
void UGP_UnitCommandComponent::HandleCommand(const FGP_UnitCommand& Command)
{
	...
	const bool bHadHeldCommand = HeldCommand.IsSet();
	uint32 PreviousSerial = 0;
	FGameplayTag PreviousTag;
	if (bHadHeldCommand)
	{
		PreviousSerial = HeldCommand.GetValue().CommandSerial;
		PreviousTag = HeldCommand.GetValue().CommandTag;
	}
