...
- This direct mirror binding is intentionally limited to the project-owned world renderer.
  Ordinary HUD widgets still consume ViewModels or HUD-root presenter accessors. The minimap
  HUD seam is `UGP_MinimapPresenter` (not a 4-million-cell ViewModel copy) plus `UGP_MinimapWidget`
  for the static background + bounded FoW overlay. The presenter binds the same
  trusted `UGP_LocalFoWComponent` as `UGP_FoWViewModelAdapter` and queries that mirror by world
  location. Coarse Revision drives `OnMinimapPresentationChanged`; there is no widget Tick and
  no 10 Hz poll.
...
- The TEMP HUD is retired. Production HUD is the active match HUD and remains **PARTIAL**. Native
  ...
  are **not** operator-validated. Native minimap presentation is complete
  ...
  2000×2000 FoW grid. It   is **not** SceneCapture, live camera, or render-target terrain. The native MVP minimap surface
  (background + FoW crop + team-colored blips + projected camera footprint + LMB click-to-pan) is
  implemented. Operator inserts `UGP_MinimapWidget` into protected `WBP_GP_HUD`.
  `WBP_GP_HUD` remains uncommitted.
  Still not implemented: minimap last-known / minimap drag-pan / Ferronite-deposit / SWARM-pulse
  markers, notifications, and production end-of-match screen.
...
UGP_SelectionComponent (local)       ─►    OnSelectionChanged    ─►   UGP_SelectionViewModel ◄─►  authored Selection/Info (operator WBP)
...
1. **VM populated by VM-owner adapter in local-player presentation lifetime.** Production Resource/Match/
   Selection VMs and adapters are owned by `UGP_HUDViewModelSubsystem`; specialized FoW presentation retains its
...
| `UGP_SelectionViewModel` | `UGP_SelectionComponent.{SelectedUnits, InspectedTarget}` (local PC) plus GAS health/max-health delegates and Worker `UGP_CargoComponent.OnCargoAmountChanged` for single presentation. Native foundation: None/Single/Group, inspect fallback, event-driven vitals/cargo. Single FieldNotify now includes `Icon` (`UTexture2D*` from `UGP_UnitDefinition::PresentationIcon`) and `AttackRange` (`UGP_UnitDefinition::AttackRangeCm`, cm). Group rows include per-actor `Icon`. Null icon is valid (placeholder until authored). No `LoadSynchronous`. Operator PIE **PASSED** for Manual slot `GP_SelectionViewModel` Single bindings (Worker / Salvage Walker / MainBase, CurrentHealth, Damage, live Worker cargo) and for Group 8×3 rows plus row-click → single selection. `WBP_GP_HUD` remains operator-local and uncommitted. | `UGP_SelectionViewModelAdapter` (`UGP_HUDViewModelSubsystem`) | Authored bottom-center Selection/Info (`WBP_GP_HUD` Manual slot `GP_SelectionViewModel`). Context Action Grid is `UGP_ContextActionPresenter`. |
...
| `UGP_MinimapVM` | Superseded as the production HUD path. Native `UGP_MinimapPresenter` on `UGP_HUDViewModelSubsystem` is the minimap presentation foundation. `UGP_MinimapWidget` is the native UMG surface: static authored background (`UGP_UIPresentationSettings::MinimapBackgroundTexture`, async only) plus a bounded FoW downsample overlay, team-colored blips, a projected camera viewport footprint outline, and LMB click-to-pan on the actual MapDest. Displayed XY = resolved camera/playable bounds; FoW grid metadata stays separate. Blips use the existing unit presentation registry. Color is `UGP_GameplayPresentationSettings::GetTeamColor(TeamId)`; Unit vs Building differs by marker size. Friendly actors are always shown inside playable bounds; enemy actors only while currently Visible. Camera indicator is the four-corner deprojected viewport on the camera pawn XY-anchor plane (not a fixed square, not team-colored). LMB inside MapDest pans the camera pawn XY-anchor (zoom/yaw/pitch/Z preserved, no RPC). Letterbox/padding is consumed without a world mapping. It does **not** snapshot child widgets per actor or copy the FoW cell arrays. | `UGP_MinimapPresenter` binds `UGP_LocalFoWComponent::OnLocalFoWUpdated`, `AGP_CameraPawn::OnResolvedCameraBoundsChanged`, `AGP_CameraPawn::OnCameraPresentationChanged`, and `UGP_LocalFoWUnitPresentationSubsystem` registry/movement delegates; widget binds `OnMinimapPresentationChanged` / `OnMinimapBlipsChanged` / `OnMinimapCameraFootprintChanged`; Slate LMB uses a weak `CreateUObject` callback into `UGP_MinimapWidget` → `PanCameraToMinimapNormalized` → `SetCameraAnchorWorldXY` | Bottom-left minimap. Operator inserts `GP Minimap` into protected `WBP_GP_HUD`. Last-known / drag-pan remain deferred extras, not part of this stage. |
...
4. **Selection — local-only.** `UGP_SelectionViewModelAdapter` читає `UGP_SelectionComponent` напряму (local PC owns), оновлює `UGP_SelectionViewModel`. No world scan, no Tick.
...
├── BottomBar
│   ├── BottomLeft  Minimap square (`UGP_MinimapWidget` background + FoW; blips/camera/input later)
...
| Future bottom-left Minimap | Native `UGP_MinimapWidget` (UMG palette: **GP Minimap**) self-binds to `UGP_HUDViewModelSubsystem::GetMinimapPresenter()`. Background = static authored `UTexture2D` on `UGP_UIPresentationSettings` (async; empty/loading → dark fallback). FoW = presentation-only downsample (default 128², clamp 32–256) of the trusted mirror; Unexplored black, Explored dim, Visible clear. Widget-layer `Xscreen = 1 - NormalizedX`, `Yscreen = 1 - NormalizedY`; presenter world +X/+Y → normalized +X/+Y is unchanged. Shared dest rect for background, FoW, blips, camera footprint, **and LMB hit-test**. Blip color = canonical team color; type = marker size. Friendly always shown inside bounds; enemy only currently Visible. Camera indicator = projected viewport quadrilateral on the pawn XY-anchor plane (neutral outline, no fill). LMB inside MapDest: SurfaceUV → shared inverse → presenter normalized → world XY → `AGP_CameraPawn::SetCameraAnchorWorldXY` (click = camera **anchor** XY, not footprint-center compensation). Zoom/yaw/pitch/Z preserved. Letterbox LMB is handled without pan so it does not fall through to world selection. RMB is not click-to-pan. No Tick / poll / world scan / `SetInputModeUIOnly` / mouse capture. Drag-pan and last-known are deferred extras. | Trusted `UGP_LocalFoWComponent` + `UGP_LocalFoWUnitPresentationSubsystem` + `AGP_CameraPawn::OnCameraPresentationChanged` via `UGP_MinimapPresenter` |
...
**MainBase PURCHASE** (catalog + execution implemented; WBP wiring operator-local): native panel state

`Actions` → PURCHASE → `PurchaseRoot` → Units / Buildings / Defense, plus selected-item states
for Buildings/Defense.
...
`RequestPurchaseBack`, `GetPurchaseCatalogRows()`, `RequestPurchaseRowPrimary` /
`RequestPurchaseRowSecondary`, `RequestLaunchUnitShuttle`, `RequestLaunchSelectedPurchaseItem`.
Rows rebuild with `BP_OnContextActionsChanged`.

UNITS: presenter-owned local manifest (LMB +1 / RMB −1) then Launch Shuttle through existing
`RequestUnitDrop` / `Server_RequestUnitDrop`. BUILDINGS / Defensive Turret: select then LAUNCH
through existing `Server_RequestBuildingPurchase`; READY increment auto-enters placement ghost.
Wall Package LAUNCH through existing `Server_RequestWallPackagePurchase` only. Spend occurs once;
deploy does not charge again.
...
- Camera transform mirror for minimap viewport rectangle: event-driven `AGP_CameraPawn::OnCameraPresentationChanged` after zoom/pitch/yaw/pan/clamp when the presentation fingerprint actually changes. The minimap draws the projected viewport footprint (deproject 4 corners → ray vs pawn XY-anchor plane Z), not a fixed square around the pawn.
...
- Visual command cursor is a native HitTestInvisible Slate overlay (`SGPCommandCursorOverlay`) added through the local player `UGameViewportClient` (ZOrder 10000, above HUD/marquee). Hardware/game cursor is hidden while targeting. Shapes (Slate primitives, no assets): Move = four-tick crosshair + center square; AttackMove = crosshair + ring; Patrol = crosshair + two opposing arrows. Overlay is removed and the normal cursor restored on confirm, RMB, Esc, invalid selection, building placement enter, EndPlay, and PlayerController teardown. `GetCommandTargetingCursor` / hardware cursor enum is leftover only — not the production visual path.
- Notification queue.
...
### Minimap (native surface: static background + FoW overlay + blips + camera footprint + LMB click-to-pan)
...
- Native foundation is `UGP_MinimapPresenter` owned by `UGP_HUDViewModelSubsystem`.
  Event-driven from the trusted local FoW mirror, `AGP_CameraPawn::OnResolvedCameraBoundsChanged`,
  and `AGP_CameraPawn::OnCameraPresentationChanged` (fingerprint-gated; idle camera ticks do not fire).
...
- Blips (MVP): `UGP_MinimapPresenter` builds a presentation-only `FGP_MinimapBlip` snapshot
  ...
  on `AGP_UnitBase` BeginPlay/EndPlay). Snapshot stores `TeamId` + Kind; paint resolves
  `UGP_GameplayPresentationSettings::GetTeamColor(TeamId)`. Units and buildings of the same team
  share that color; buildings use a larger marker. Friendly team actors are always shown inside
  displayed camera bounds (not FoW-gated). Enemy actors use the same
  `ShouldPresentUnitForLocalPlayer` / trusted `UGP_LocalFoWComponent::IsVisible` gate as world
  presentation: Visible → show, Explored/Unexplored → omit. Invalid TeamId (< 1) is not treated
  as friendly or enemy. Last-known markers are deferred extras. Movement/visibility refresh
...
- The native MVP minimap surface is complete: static background / fallback, camera-bounds crop,
  FoW overlay, team-colored friendly/visible-enemy blips, projected camera footprint, and LMB
  click-to-pan. Deferred extras: last-known, drag-pan, RMB minimap orders, pings, minimap
  selection, per-type icons, SceneCapture, and terrain/voxel-aware footprint.
- LMB click-to-pan: `SGPMinimapSurface::OnMouseButtonDown` (LeftMouseButton only)
  uses `ComputeSharedMapDestLocal` (same as `OnPaint`). Inside MapDest: local → SurfaceUV →
  `SurfaceUVToPresenterNormalized` `(1-X, 1-Y)` → `UGP_MinimapPresenter::PanCameraToMinimapNormalized`
  → `AGP_CameraPawn::SetCameraAnchorWorldXY`. Click target = camera pawn actor XY-anchor.
  Letterbox/padding: `FReply::Handled()` without a world mapping. Missing presenter/camera/NaN:
  ignored. No RPC. Selection/marquee/RMB command IMCs are unchanged; the surface consumes LMB so
  the click does not fall through to world selection. Drag-pan is not implemented.
